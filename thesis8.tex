...
theoretical contribution is the theoretical motivation for a stochastic
...
As the primary \emph{applied contribution}, a \textbf{Hybrid SDE-ML model} is
...
The empirical results show that persistence and simple lag-based models are
very strong short-horizon baselines. The Hybrid SDE-ML framework remains
competitive in point forecasting and adds calibrated 90\,\% prediction
intervals (coverage 91.7\,\%) together with an economically interpretable
...
\begin{itemize}[leftmargin=1.25cm]
  ...
  \item to provide a theoretically motivated stochastic process representation of Bitcoin
  ...
  \item to propose and implement a practically useful \textbf{Hybrid SDE-ML model} with
...
Chapter~4 describes the ML models and the Hybrid SDE-ML model.
...
For GBM these conditions apply directly. For the Merton jump-diffusion and
Heston square-root variance model, specialised existence and uniqueness results
are required and are discussed separately.
...
\label{prop:emh}
Suppose the market for Bitcoin satisfies weak-form efficiency.  Then excess
returns are not predictable from past prices under the assumed information set,
and the log-return process $\{r_t = \ln(S_t / S_{t-1})\}$ constitutes a sequence
of random variables uncorrelated with any function of past prices.  A Markov
representation may be obtained only after specifying an appropriate state
vector, which may include volatility, liquidity, or latent regime variables.
\end{proposition}
...
\]
This is the martingale (or supermartingale) property.  It implies that past
prices do not provide a systematic forecasting advantage for excess returns
under the assumed information set.  Any excess return $r_t - \pi_{t-1}$ is
therefore unpredictable from past information, hence random.  A one-dimensional
Markov representation for price itself would require additional state
assumptions beyond weak-form EMH.
\end{proof}
...
measure (up to a risk premium).  Under the additional assumption that the
information filtration is Brownian and the martingale component is
square-integrable, the Martingale Representation Theorem (see~[4]) motivates
the stochastic integral representation above.  Assumption (A2) rules out jump
arrivals and supports a continuous Brownian filtration rather than a Poisson or
general discontinuous one.  Assumptions (A3)--(A4) ensure that the coefficients
are well-defined and the integral converges.  The argument should therefore be
read as a heuristic justification for an It\^{o} process representation rather
than as a consequence of EMH alone.
\end{proof}
...
\begin{remark}
The CIR variance process does not satisfy the global Lipschitz condition
because the square-root diffusion coefficient is non-smooth at zero.
Accordingly, existence and uniqueness for the Heston variance process follow
from specialised results for square-root diffusions under non-negativity
conditions, rather than directly from general global-Lipschitz theorems.
The Feller condition is relevant for strict positivity, but it is not itself a
...
In the main experiment, the target variable is therefore the future \emph{price
level} $S_{t+h}$ rather than the future return.  The forecasting horizon is
$h = 10$ bars, corresponding to 150 minutes on the 15-minute series.  An
alternative target not used in the primary specification is the log-return
$r_{t,t+h} = \ln(S_{t+h}/S_t)$.  Because the target is the future price level,
high $R^2$ values should be interpreted cautiously: a substantial part of the
apparent explanatory power may come from price persistence.
...
The Hybrid SDE-ML model is conceptually related to Neural SDEs~[14], since a
component of the SDE dynamics is parameterised by a data-driven function
approximator. However, it is not a full Neural SDE framework: the drift and
diffusion are not jointly learned as continuous-time neural networks, and the
model is not trained via likelihood-based or pathwise Neural SDE objectives.
Instead, our approach uses a gradient boosting regressor for the drift and an
analytic realised-variance estimator for the diffusion. This provides
interpretable feature importances for the drift component
(Table~\ref{tab:feat_imp}) while retaining the theoretical guarantees of GBM
...
data spans 2014-04-16 to 2026-04-14 (4\,365 daily observations after
preprocessing).  The primary
...
The choice $h = 10$ corresponds to a 150-minute forecast horizon.  This horizon
is long enough to avoid a trivial one-bar-ahead nowcast, but still short enough
to remain relevant for intraday Bitcoin dynamics and for the available 15-minute
feature set.  It therefore provides a practical middle ground between very
short-horizon microstructure noise and much longer horizons where the current
intraday covariates become less informative.

At forecast origin $t$, all features are constructed from information available
at the close of bar $t$.  In particular, $H_t$, $L_t$, $S_t$, and $V_t$ are used
only as end-of-bar quantities, so the specification does not assume intrabar
knowledge when forming the prediction.  This timing convention is important for
avoiding look-ahead bias in the intraday setting.
...
For the LSTM specifically, scaling is fit on the training portion of each fold
only, the validation split is taken only from the training fold, and the test
fold is never used for early stopping.  This protocol is designed to minimise
data leakage in the sequential setting.

This asymmetry should be interpreted carefully.  The SDE models are structural
benchmarks and are intentionally calibrated from the price process alone,
without additional explanatory variables such as volume or technical signals.
The ML models, by contrast, are designed to exploit a richer information set.
Therefore, the purpose of the comparison is not to claim that all model classes
operate under identical informational conditions, but to analyse what each
paradigm can deliver under its natural assumptions.  In this sense, the hybrid
models provide the most informative bridge: they retain an SDE-based dynamic
structure while using a feature set comparable to the one used by the ML
baselines.
...
that later folds test on more recent, potentially more volatile data.  Fold 5
contains 568 test bars rather than 566 because the remainder from partitioning
the second half of the sample is allocated to the final fold.
...
In the final implementation, the Heston model is calibrated directly on the
15-minute series used for forecasting, with $\Delta t = 1/96$ day, rather than
being estimated on daily data and then mechanically rescaled to intraday
frequency.  The moment-matching procedure is applied to rolling realised
variance computed at the same sampling frequency.  This makes the comparison
with the 15-minute ML and Hybrid forecasts methodologically cleaner.

Using the 30-bar rolling realised variance series, the calibration yields
$\hat\kappa = 0.231$, $\hat\theta = 0.00176$, $\hat\xi = 0.0124$,
$\hat\rho = -0.132$.  In all SDE forecast experiments, 1\,000 Monte Carlo
paths are simulated per forecast origin, and point predictions are taken from
the median simulated path.  The implied variance half-life is
$\ln 2 / \hat\kappa \approx 3$ trading days.
...
\midrule
Elastic Net             & 291.2  & 418.7  & 0.91 & -24.68 \\
Random Forest           & 352.5  & 481.3  & 0.88 & --- \\
Gradient Boosting       & 351.6  & 482.3  & 0.88 & -24.09 \\
\textbf{SDE-ML Hybrid}  & \textbf{288.1} & \textbf{414.7} & \textbf{0.91} & \textbf{-9.98} \\
Naive persistence ($\hat S_{t+h} = S_t$) & 397.2 & 564.0 & 0.84 & -17.76 \\
\midrule
GBM (SDE)               & 290.9 & 417.3 & 0.91 & --- \\
Merton Jump-Diff.\ (SDE) & 288.8 & 414.7 & 0.91 & --- \\
Heston (SDE)            & 294.7 & 421.5 & 0.91 & --- \\
\midrule
Buy \& Hold             & ---    & ---    & ---   & -5.23 \\
\bottomrule
\end{tabular}
...
Since Bitcoin prices are highly persistent, all ML and Hybrid models are also
compared against a naive persistence-style baseline in the notebook.  In the
final rerun, this baseline attains MAE\,=\,397.2 and RMSE\,=\,564.0, which is
materially worse than both the best Hybrid model (MAE\,=\,288.1) and the best
tabular ML model (Elastic Net, MAE\,=\,291.2).  At the same time, this
comparison should be interpreted cautiously: the current notebook implementation
of the baseline needs to be kept fully aligned with the intended definition of a
last-value persistence forecast before strong methodological claims are made.

Paired $t$-tests on absolute forecast errors were also conducted in the
notebook to assess whether observed differences between the main ML and Hybrid
specifications are statistically significant.  In the final single-split
experiment, the Hybrid Gradient-Boosting variant significantly outperforms both
the pure Gradient Boosting regressor ($t = -22.58$, $p < 0.001$) and the naive
persistence baseline ($t = -19.22$, $p < 0.001$).  These tests are used as a
robustness check only; because forecast errors may be serially correlated, the
results should be interpreted more cautiously than a fully robust
forecast-comparison test such as Diebold--Mariano.

\subsection{Walk-Forward Cross-Validation Results}

Table~\ref{tab:wf_results} reports the cross-validation mean and standard
deviation of MAE across the five expanding-window folds.  Compared to the
single split, the ranking changes materially, which highlights the importance of
time-ordered re-estimation rather than relying on a single chronological split.

\begin{table}[H]
\begin{singlespace}
\fontsize{10}{11}\selectfont
\caption{Walk-forward cross-validation: MAE summary (USD, 15-minute data)}
\label{tab:wf_results}
\begin{tabular}{@{}lrr@{}}
\toprule
Model & Mean MAE & Std MAE \\
\midrule
Naive persistence & 326 & 51 \\
Hybrid (GB drift)  & 329 & 52 \\
Hybrid (EN drift)  & 329 & 52 \\
Hybrid (RF drift)  & 329 & 52 \\
Elastic Net        & 332 & 57 \\
Gradient Boosting  & 1\,097 & 749 \\
Random Forest      & 1\,153 & 715 \\
\bottomrule
\end{tabular}
\end{singlespace}
\end{table}

The walk-forward results remain informative but materially change the ranking
observed under the single split.  The naive persistence benchmark achieves the
lowest mean MAE (326 USD), while all three Hybrid variants cluster very closely
behind it at approximately 329 USD.  Elastic Net remains competitive
(332 USD), whereas the pure Gradient Boosting and Random Forest regressors
degrade substantially under expanding-window evaluation.  This pattern suggests
that on short-horizon intraday Bitcoin forecasting, persistence is a very hard
baseline to beat consistently across time, and the main value of the Hybrid
approach lies in remaining competitive while preserving a stochastic
...
The backtest is intentionally stylised.  Signals are formed at the close of bar
$t$ using information available at that time, and the resulting position is
evaluated over the subsequent forecast horizon.  The comparison is therefore
useful for relative model ranking, but it is not a full market-microstructure
simulation with transaction-cost sensitivity, order-book dynamics, or execution
delay modelling.
...
\toprule
Strategy & Total Return (\%) & Sharpe & Max DD (\%) & Turnover & Trades & PnL (USD) \\
\midrule
Buy \& Hold              & $-4.83$ & -5.23 & $-7.70$ & 1.00 & 1 & $-483.00$ \\
Elastic Net              & $-16.93$ & -24.68 & --- & --- & --- & $-1\,693.00$ \\
Gradient Boosting        & $-16.06$ & -24.09 & --- & --- & --- & $-1\,606.00$ \\
Naive persistence        & $-12.34$ & -17.76 & --- & --- & --- & $-1\,234.00$ \\
\textbf{SDE-ML Hybrid}   & $\mathbf{-7.22}$ & $\mathbf{-9.98}$ & --- & --- & --- & $\mathbf{-722.00}$ \\
\bottomrule
\end{tabular}
\end{singlespace}
\end{table}
...
In the final rerun of the notebook, the stylised momentum backtest is negative
for all reported strategies, including buy-and-hold.  The Hybrid SDE-ML model
still performs best among the predictive strategies in relative terms, with
total return $-7.22\,\%$ versus $-12.34\,\%$ for naive persistence,
$-16.06\,\%$ for Gradient Boosting, and $-16.93\,\%$ for Elastic Net.  Its
Sharpe ratio remains the least negative among the model-driven strategies
reported here (Hybrid: $-9.98$; Naive: $-17.76$; Elastic Net: $-24.68$;
Gradient Boosting: $-24.09$), although buy-and-hold remains better at $-5.23$.

These results reinforce the need for caution: statistical improvements in price
prediction do not automatically translate into profitable trading performance,
especially under a stylised long-only decision rule over a short intraday
sample.
...
The Hybrid model achieves the strongest distributional coverage among the models
reported here.  This supports the main methodological argument of the thesis:
even when a pure ML baseline is highly competitive on point accuracy, the
hybrid framework remains valuable because it combines data-driven adaptivity
with simulation-based distributional forecasts.

\section{Comparative Analysis}

\subsection{Predictive Accuracy}

The empirical comparison should be read at two levels.  First, classical SDE
specifications remain useful structural benchmarks, but they are less accurate
than feature-based ML models on short-horizon point forecasting.  Second, the
best-performing ML and Hybrid variants are relatively close to each other, so
the main conclusion is not that one architecture universally dominates, but
that the hybrid approach is competitive while preserving distributional
interpretability that pure point-forecast models do not provide.
...
\begin{enumerate}[leftmargin=1.25cm, label=\arabic*.]
  \item \textbf{Theoretical motivation.}  Via a series of propositions, we
    motivated a stochastic process representation for Bitcoin prices under
    additional modelling assumptions, and showed that the observed stylised
  ...
  \item \textbf{SDE calibration.}  All three SDE models were successfully
    calibrated, with the final Heston implementation estimated directly at the
    15-minute frequency used in forecasting rather than via daily-to-intraday
    rescaling.  The Merton model best fits the empirical
    return distribution (KS $p = 0.11$) and achieves closest-to-nominal
    prediction interval coverage (91.3\,\%).

  \item \textbf{ML models remain strong short-horizon baselines.}  On the
    15-minute forecasting task, simple lag-based machine learning models are
    difficult to beat.  This is an important empirical result, because it shows
    that short-horizon Bitcoin prediction is highly sensitive to feature timing,
    horizon definition, and evaluation protocol.

  \item \textbf{Walk-forward validation.}  The walk-forward results show that
    persistence is the strongest baseline under expanding-window evaluation
    (mean MAE 326 USD), while all three Hybrid variants remain very close at
    approximately 329 USD.  Elastic Net follows at 332 USD, whereas the pure
    tree-based tabular models deteriorate sharply.  This confirms that the
    single-split ranking is not fully stable across time and that persistence
    must be treated as a primary benchmark rather than a cosmetic reference.

  \item \textbf{Hybrid SDE+ML models provide a meaningful methodological
    bridge.}  The implemented hybrid family combines a stochastic-process view
    of price dynamics with a data-driven approximation of the drift component.
    Even when a hybrid model is not the best model under every point metric, it
    remains valuable because it preserves distributional interpretation while
    improving adaptivity relative to classical constant-coefficient SDEs.

  \item \textbf{Economic evaluation is informative but simplified.}  The
    trading comparison shows that predictive differences can translate into
    different Sharpe ratios and drawdowns, but these results should be treated
    as comparative indicators rather than as direct evidence of deployable
    profitability.
...
The hybrid SDE+ML framework addresses this tension by using machine learning to
parameterise selected components of an otherwise interpretable stochastic model.
The main contribution of the thesis is therefore not only a single numerical
result, but a reproducible framework for studying how structural stochastic
models and flexible data-driven models can be combined.  This framing is more
accurate and more defensible than the stronger claim that one hybrid variant
must dominate all baselines under every metric and every split.

\section*{Limitations}

The study has several important limitations.  First, the intraday sample is
short because Yahoo Finance retains only a limited history for 15-minute data.
Second, the primary target is the future price level rather than the future
return, so high $R^2$ values may overstate genuine predictability because of
price persistence.  Third, the comparison is not information-set equivalent:
the SDE models are calibrated only from the return process, whereas the ML and
Hybrid models use a richer feature set including price lags, volume, high/low,
and technical indicators.  The empirical results should therefore be interpreted
as a comparison of modelling paradigms under their natural inputs rather than as
a fully controlled model-class comparison.  Fourth, the economic backtest is
stylised and does not model the full range of market frictions, execution
timing, or transaction-cost sensitivity.  Fifth, the Heston calibration is
moment-based rather than likelihood-based.  Finally, the proposed Hybrid model
is related to Neural SDEs only conceptually and is not methodologically
equivalent to a full neural SDE framework.  All findings are specific to
Bitcoin and to the chosen sample period.

\section*{Reproducibility}

The empirical pipeline is implemented in Python using standard scientific and
machine-learning libraries, including \texttt{numpy}, \texttt{pandas},
\texttt{scikit-learn}, \texttt{tensorflow/keras}, \texttt{scipy}, and
\texttt{yfinance}.  Data were downloaded from Yahoo Finance and evaluated using
chronological train-test splits together with five-fold expanding-window
walk-forward validation on the 15-minute series.  Where stochastic training is
involved, fixed random seeds should be used to improve reproducibility.  The
main model hyperparameters, forecasting horizon, and fold structure are reported
in Chapters~4--6 so that the experiments can be replicated from the attached
code and notebook workflow.
...
\begin{enumerate}[leftmargin=1.25cm, label=\arabic*.]
  ...
  \item \textbf{Full Neural SDE.}  Parameterising both drift \emph{and}
    ...
    adaptivity.
  \item \textbf{Kalman filtering and state-space estimation.}  A Kalman-filter
    layer or a more general state-space formulation could improve latent-state
    estimation and provide a cleaner connection between noisy observations and
