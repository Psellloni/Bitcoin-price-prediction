...
The empirical results show that feature-based ML models are very strong
short-horizon baselines, while the Hybrid SDE-ML framework remains competitive
and adds calibrated 90\,\% prediction intervals (coverage 91.7\,\%) together
with an economically interpretable stochastic structure.
...
For interpretation, a naive persistence benchmark is also computed in the
experimental notebook: the forecast simply sets $\hat S_{t+h} = S_t$.  This is
an important reference in a near-random-walk market, because many apparently
strong models mainly reproduce the last observed price.  The feature-importance
results in Section~\ref{tab:feat_imp} should therefore be read relative to this
...
specifications are statistically significant.  Because the exact $p$-values are
reported in the computational notebook rather than reproduced here, the thesis
uses these tests as a robustness check and avoids stronger claims than the
...
MAE (356 USD) with comparable variability in this table, but the broader
interpretation should remain cautious: on short-horizon Bitcoin forecasting,
small differences between strong lag-based models may not always translate into
statistically decisive improvements once paired error tests are applied.
...
economically meaningful improvements in portfolio risk management within this
stylised backtest.  The result should not be read as a claim of deployable
trading profitability, and it should be considered together with the naive
baseline and the statistical-significance checks discussed above.
